# DockControl — Guide utilisateur

Guide complet pour déployer apps, bases de données, domaines et microservices sur DockControl.
...
DockControl fonctionne dans l'un de deux modes, défini dans **Réglages → Infrastructure** :
...
| **Multi-serveur** | Ce VPS + d'autres VPS connectés via l'agent DockControl. Les apps peuvent être déployées sur n'importe quel serveur enregistré. | Plusieurs VPS, régions différentes, isolation de charges. |
...
2. DockControl génère une commande d'installation (one-liner avec token)
3. SSH dans le VPS distant, colle la commande
4. L'agent DockControl s'installe, se connecte, et le serveur passe à `ONLINE`
...
DockControl donne à chaque app :

- Un nom de container prédictible : `dockcontrol-<slug>`
- Un nom d'hôte interne prédictible : identique au nom de container
- L'appartenance au réseau Docker partagé du projet

Les autres apps du même projet l'atteignent à `http://dockcontrol-<slug>:<port>` — aucune URL publique requise.
...
DockControl crée un container `dockcontrol-db-<slug>` sur le réseau du projet, avec username/password générés.
...
Tu n'as pas besoin d'"acheter" le domaine via DockControl — tu dis juste à DockControl que tu le possèdes. DockControl :
...
La page Domaines groupe visuellement les sous-domaines sous leur apex. Clique une card pour déployer ses sous-domaines. "Add subdomain" pré-remplit le parent (tu tapes juste `api`, DockControl ajoute `.athexis.xyz`).

---

## 6. Microservices

Microservices dans DockControl = plusieurs apps dans le même projet qui se parlent.

### Comment ça marche

Chaque projet possède un réseau Docker : `dockcontrol_proj_<projectId>`.

Quand tu déploies une app dans un projet :

- **Mode Local** : l'API crée le réseau (si manquant) et y attache l'app.
- **Mode Multi-serveur (même serveur)** : l'agent DockControl crée le réseau et écrit un `docker-compose.override.yml` qui attache chaque service de la stack. Pas de config manuelle.
...
|---|---|---|
| `frontend` | App Next.js, port 3000 | `dockcontrol-frontend:3000` |
| `api` | App NestJS, port 4000 | `dockcontrol-api:4000` |
...
Attache un serveur mail à un domaine → DockControl te dit les records MX / A / SPF / DKIM / DMARC / PTR exacts à ajouter. L'onglet "DNS health" montre ce qui manque.
