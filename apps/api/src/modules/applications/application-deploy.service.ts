  ...
  async runPhpSiteDeploy(
    ...
    try {
      ...
      if (webServer === 'nginx') {
        ...
        // FastCGI to the UNIQUE fpm container_name (not the shared `app` service
        // alias) so Docker DNS can't round-robin to another tenant's container.
        fs.writeFileSync(path.join(appDir, 'default.conf'), buildNginxConf(`${containerNm}-fpm`, phpIni));
