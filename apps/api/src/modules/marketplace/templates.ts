...
networks:
  ...
  wordpress:
    ...
    volumes:
      ...
      # User-editable php.ini overrides (PHP configuration card).
      - __HOST_APP_DIR__/zz-dockcontrol-php.ini:/usr/local/etc/php/conf.d/zz-dockcontrol-php.ini:ro
    depends_on:
      - wordpress-db
  wordpress-db:
    image: mariadb:11
...
networks:
  ...
  prestashop:
    ...
    volumes:
      ...
      # User-editable php.ini overrides (PHP configuration card). Loads before
      # the zzz- proxy drop-in; empty unless the user sets directives.
      - __HOST_APP_DIR__/zz-dockcontrol-php.ini:/usr/local/etc/php/conf.d/zz-dockcontrol-php.ini:ro
    depends_on:
...
`,
    ...
    // writer wins on auto_prepend_file and short_open_tag.
    'php-trust-proxy.ini':
      `; Auto-generated by DockControl marketplace install.
auto_prepend_file = /usr/local/etc/php/dockcontrol-trust-proxy.php
; PrestaShop's installer requires short_open_tag = Off (it warns otherwise).
; Harmless for the modern PrestaShop codebase, which uses full <?php tags.
short_open_tag = Off
`,
...
// ── PHP marketplace apps ────────────────────────────────────────────────
// Marketplace apps that run on an official PHP image and therefore accept a
// php.ini override drop-in (the "PHP configuration" card on the app page).
// `service` is the compose service the .ini must mount into; `confDir` is the
// image's PHP conf.d. The generated drop-in is shipped as a side-file and
// bind-mounted read-only — same mechanism as PrestaShop's proxy side-files.
export const PHP_INI_SIDEFILE = 'zz-dockcontrol-php.ini';

export const PHP_MARKETPLACE_APPS: Record<string, { service: string; confDir: string }> = {
  prestashop: { service: 'prestashop', confDir: '/usr/local/etc/php/conf.d' },
  wordpress: { service: 'wordpress', confDir: '/usr/local/etc/php/conf.d' },
  phpmyadmin: { service: 'phpmyadmin', confDir: '/usr/local/etc/php/conf.d' },
  nextcloud: { service: 'nextcloud', confDir: '/usr/local/etc/php/conf.d' },
};

/**
 * The bind-mount line a PHP marketplace compose carries for the .ini drop-in.
 * `hostDir` is the bind SOURCE base: the install-time `__HOST_APP_DIR__`
 * placeholder for template rendering, OR an already-resolved absolute host path
 * when injecting into a live install's compose (the placeholder is only
 * substituted at install time, so a late injection MUST pass the real path).
 */
export function phpIniMountFor(slug: string, hostDir = '__HOST_APP_DIR__'): string | null {
  const def = PHP_MARKETPLACE_APPS[slug];
  if (!def) return null;
  return `${hostDir}/${PHP_INI_SIDEFILE}:${def.confDir}/${PHP_INI_SIDEFILE}:ro`;
}
