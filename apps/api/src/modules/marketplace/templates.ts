...
# Tell PHP to auto-prepend our trust-proxy shim on every request. The
# file is bind-mounted to /usr/local/etc/php/conf.d/, which the PHP
# image's stock entrypoint scans on boot. \`auto_prepend_file\` runs
# before any PrestaShop code (including config/config.inc.php), so by
# the time Tools::usingSecureMode() looks at \$_SERVER['HTTPS'], it is
# already set to 'on' for requests that arrived over HTTPS upstream.
`,
